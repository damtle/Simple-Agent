...
Parser 只检查输出格式和 Action Protocol，不执行工具，也不替模型
猜测、改写行动名称或参数语义。
"""
...
def validate_attraction_arguments(
    ...
    """校验 get_attraction_info 的 name 参数。"""
    validate_exact_fields(
        data=arguments,
        expected={"name"},
...
def validate_calculator_arguments(
    ...
    """校验 calculator 的 operation、a 和 b 参数。"""
    validate_exact_fields(
        data=arguments,
